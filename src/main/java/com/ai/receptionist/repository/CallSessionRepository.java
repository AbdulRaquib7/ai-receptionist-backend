...
public interface CallSessionRepository extends JpaRepository<CallSession, Long> {
    ...
    @Query(value = "SELECT COALESCE(metadata->>'pipeline', 'UNKNOWN') AS pipeline, COUNT(*) " +
            "FROM call_session WHERE started_at >= :since " +
            "AND (:tenantId IS NULL OR tenant_id = :tenantId) " +
            "GROUP BY COALESCE(metadata->>'pipeline', 'UNKNOWN')",
            nativeQuery = true)
    List<Object[]> countByPipeline(@Param("since") Instant since, @Param("tenantId") Long tenantId);
}
