...
public class TenantService {
    ...
    /**
     * Find tenant by slug. Returns null if not found.
     */
    ...
    /**
     * Find tenant by ID. Returns null if not found.
     */
    ...
    /**
     * Single access point for checking if a tenant uses the OpenAI Realtime API pipeline.
     * Checks global kill-switch first, then per-tenant DB flag.
     */
    public boolean isRealtimeApiEnabled(Tenant tenant) {
        if (!realtimeApiProperties.isEnabled()) {
            // Global kill-switch is off — no tenant uses Realtime API
            return false;
        }
        boolean enabled = tenant != null && tenant.isUseRealtimeApi();
        log.debug("Tenant [{}] realtime API flag: {}", tenant != null ? tenant.getSlug() : "null", enabled);
        return enabled;
    }

    /**
     * Toggle the Realtime API flag for a tenant and persist immediately.
     * Clears the tenant cache so the next call reads the fresh value.
     */
    public Tenant setRealtimeApiEnabled(String slug, boolean enabled) {
        Tenant tenant = tenantRepository.findBySlug(slug)
                .orElseThrow(() -> new IllegalArgumentException("Tenant not found: " + slug));
        tenant.setUseRealtimeApi(enabled);
        tenantRepository.save(tenant);
        // Invalidate cache so next call sees the updated flag
        tenantByPhone.invalidate(tenant.getTwilioPhone());
        log.info("Tenant [{}] realtime API flag updated to {}", slug, enabled);
        return tenant;
    }
